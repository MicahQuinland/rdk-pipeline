...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        ...
        my_eval = {
            ...
            'Annotation': 'Lambda Functions must not be exposed via Function URL use API gateway',
            'OrderingTimestamp': str(datetime.datetime.now())
        }

        try:
            response = lambda_client.get_function_url_config(
                FunctionName=configuration_item['ARN']
            )

            if response['FunctionUrl']:
                return my_eval
            else:
                my_eval['Annotation'] = 'NON_COMPLIANT Lambda Function URL error'
                return my_eval
            
        except Exception as e:
            # We expect/want their to be no resource found, in this case the Lambda Function is Complaint
            return 'COMPLIANT'
