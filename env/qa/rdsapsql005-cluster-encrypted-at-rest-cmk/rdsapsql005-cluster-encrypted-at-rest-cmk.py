...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        my_eval = {
            ...
            'Annotation': 'Amazon RDS Cluster is not storage encrypted with CMK (Customer Managed Key) and/or not managing master credentials with secrets manager using CMK (Customer Managed Key)',
            'OrderingTimestamp': str(datetime.datetime.now())
        }

        #Check that storage is encrypted and master credentials use secrets manager
        if configuration_item['configuration']['storageEncrypted'] == False or 'masterUserSecret' not in configuration_item['configuration']:
            return my_eval

        #Check that the KeyIds used are not AWS managed
        if 'kmsKeyId' in configuration_item['configuration'] and 'kmsKeyId' in configuration_item['configuration']['masterUserSecret']:
            kms_client = get_client("kms", event)
            rds_managed_ky_id = ''
            secretsmanager_managed_ky_id = ''
            storage_key_id_in_use = configuration_item['configuration']['kmsKeyId'].split("/")[1]
            masterUserSecret_key_id_in_use = configuration_item['configuration']['masterUserSecret']['kmsKeyId'].split("/")[1]
            
            response = kms_client.list_aliases(Limit=100)

            for x in response['Aliases']:
                if x['AliasName'] == 'alias/aws/rds':
                    rds_managed_ky_id = x['TargetKeyId']
                if x['AliasName'] == 'alias/aws/secretsmanager':
                    secretsmanager_managed_ky_id = x['TargetKeyId']

            while 'NextMarker' in response:
                response = kms_client.list_aliases(Limit=100,Marker=response['NextMarker'])  
                for x in response['Aliases']:
                    if x['AliasName'] == 'alias/aws/rds':
                        rds_managed_ky_id = x['TargetKeyId']
                    if x['AliasName'] == 'alias/aws/secretsmanager':
            ...
            if storage_key_id_in_use == rds_managed_ky_id or masterUserSecret_key_id_in_use == secretsmanager_managed_ky_id:
                return my_eval
            else:
                return 'COMPLIANT'
        else:
