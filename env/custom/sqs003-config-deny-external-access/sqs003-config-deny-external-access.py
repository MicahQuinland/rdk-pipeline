...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        my_eval = {
            ...
            'Annotation': 'Amazon SQS should not allow external account access via access policy',
            'OrderingTimestamp': str(datetime.datetime.now())
        }

        compliance_list = []

        if 'Policy' in configuration_item['configuration']:
            access_policy = json.loads(configuration_item['configuration']['Policy'].replace("\\", ""))
            awsAccountId = configuration_item['awsAccountId']
            for x in access_policy['Statement']:
                if 'AWS' in x['Principal']:
                    def sub_compliance(split_arn):
                        if awsAccountId in split_arn:
                            compliance_list.append('COMPLIANT')
                        else:
                            compliance_list.append('NON_COMPLIANT')
                    if isinstance(x['Principal']['AWS'], str):
                        sub_compliance(x['Principal']['AWS'].split(":"))
                    if isinstance(x['Principal']['AWS'], list):
                        for y in x['Principal']['AWS']:
                            sub_compliance(y.split(":"))
                else:
                    return 'COMPLIANT'
        else:
            return 'COMPLIANT'
        ...
        print("Exception:" + str(e) + "\nSQS-Queue: " + configuration_item['resourceName'])
        raise
