...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        my_eval = {
            ...
            'Annotation': 'X-Ray tracing must be enabled for Step Functions',
            'OrderingTimestamp': str(datetime.datetime.now())
        }
        if 'configuration'in configuration_item and 'TracingConfiguration' in configuration_item['configuration'] and 'Enabled' in configuration_item['configuration']['TracingConfiguration']:
            if configuration_item['configuration']['TracingConfiguration']['Enabled']:
                return 'COMPLIANT'
        else:
            return my_eval

    except Exception as e:
        print("Exception:" + str(e) + "\nStateMachine: " + configuration_item['configuration']['StateMachineName'])
        raise
