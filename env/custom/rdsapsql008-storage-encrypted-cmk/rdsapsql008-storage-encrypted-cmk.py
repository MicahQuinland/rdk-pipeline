...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        my_eval = {
            ...
            'Annotation': 'Amazon RDS Cluster is not storage or performanceInsights encrypted with CMK (Customer Managed Key)',
            'OrderingTimestamp': str(datetime.datetime.now())
        }

        #Check that storage is encrypted
        if configuration_item['configuration']['storageEncrypted'] == False:
            return my_eval

        compliance_list = []
        kms_client = get_client("kms", event)
        rds_managed_ky_id = ''
        response = kms_client.list_aliases(Limit=5)
        ...
        #Check that the Storage KeyId used is not AWS managed
        if 'kmsKeyId' in configuration_item['configuration']:
            storage_key_id_in_use = configuration_item['configuration']['kmsKeyId'].split("/")[1]
            if storage_key_id_in_use == rds_managed_ky_id:
                compliance_list.append('NON_COMPLIANT')   
            else:
                compliance_list.append('COMPLIANT')
        else:
            return my_eval
        
        #Check that the optional performance insights also adhere's to CMK standard
        if configuration_item['configuration']['performanceInsightsEnabled'] == True and 'performanceInsightsKMSKeyId' in configuration_item['configuration']:
            insights_key_id_in_use = configuration_item['configuration']['performanceInsightsKMSKeyId'].split("/")[1]
            if insights_key_id_in_use == rds_managed_ky_id:
                compliance_list.append('NON_COMPLIANT')   
            else:
                compliance_list.append('COMPLIANT')
        
        if 'NON_COMPLIANT' in compliance_list or not compliance_list:
            return my_eval
        else:
            return 'COMPLIANT'
