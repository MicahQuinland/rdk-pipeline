    ...
    try:
        ...
        for x in policy_doc['Statement']:
            if 'Service' in x['Principal']:
                my_eval['Annotation'] = 'Must include aws:SourceArn & aws:SourceAccount Condition in Service trust policy'
                if 'Condition' in x:
                    if 'ArnLike' in x['Condition'] and 'StringEquals' in x['Condition']:
                        if 'aws:SourceArn' in x['Condition']['ArnLike'] and 'aws:SourceAccount' in x['Condition']['StringEquals']:
                ...
                my_eval['Annotation'] = 'Must include sts:ExternalId Condition in AWS trust policy'
                if 'Condition' in x:
                    if 'StringEquals' in x['Condition']:
                        if 'sts:ExternalId' in x['Condition']['StringEquals']:
                            compliance_list.append('COMPLIANT')
                        else:
                            compliance_list.append('NON_COMPLIANT')                     
                    else:
                    ...
                    compliance_list.append('NON_COMPLIANT')
            #
            # Need further conversation to specify Federated Condition for IAM trust policies
            #
            # elif 'Federated' in x['Principal']:
            #     my_eval['Annotation'] = 'Must include Condition in Federated trust policy'
            #     if 'Condition' in x:
            #         if 'StringEquals' in x['Condition']:
            #             my_eval['ComplianceType'] = 'COMPLIANT'
            #             my_eval['Annotation'] = ' '
            #             return my_eval                        
            #         else:
            #             return my_eval
        ...
        print("Exception:" + str(e) + "\nIAMRole: " + configuration_item['configuration']['arn'])
        raise
